...
CREATE TABLE ndvi_raster_with_properties (
    ...
    year int,
    formal_green int,
    public_access_closest_buffer int,
    area double precision,
    rast raster
);

INSERT INTO ndvi_raster_with_properties(poly_id, raster_id, year, formal_green, public_access_closest_buffer, area, rast)
SELECT 
    p.ogc_fid AS poly_id,
    r.rid AS raster_id,
    r.year,
    p.formal_green,
    p.public_access_closest_buffer,
    r.area,
...
CREATE INDEX ndvi_raster_clipped_public_idx ON ndvi_raster_with_properties(public_access_closest_buffer);
